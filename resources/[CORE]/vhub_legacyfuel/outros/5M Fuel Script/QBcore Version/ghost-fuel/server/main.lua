...
RegisterServerEvent('ghost-fuel:server:Pay', function(amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveMoney("cash", amount, "Fuel buy")
end)

RegisterServerEvent('ghost-fuel:server:GiveJerrican', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ghost-fuel:server:AttachRope', function(netIdProp, coordPumps, model)
	local src = source
    local citizenid = QBCore.Functions.GetPlayer(src).PlayerData.citizenid    
    TriggerClientEvent('ghost-fuel:client:AttachRope', -1, netIdProp, coordPumps, model, citizenid)
end)

RegisterServerEvent('ghost-fuel:server:DetachRope', function(src)
	local srctemp = source
    local citizenid = QBCore.Functions.GetPlayer(srctemp).PlayerData.citizenid    
    TriggerClientEvent('ghost-fuel:client:DetachRope', -1, citizenid, src)
end)

RegisterNetEvent('ghost-fuel:server:UpdateVehicleDateTimeIn', function(plate)
    MySQL.update('UPDATE player_vehicles SET datetimein = ? WHERE plate = ?', {os.time(), plate})
end)

QBCore.Functions.CreateCallback('ghost-fuel:server:GetTimeInGarage', function(source, cb, plate)
...
QBCore.Commands.Add('fuel', "Set fuel/charge for vehicle", {{name='amount', help='Amount of fuel in %'}}, false, function(source, args)
    ...
    TriggerClientEvent("ghost-fuel:SetFuel", src, amount)
end, 'admin')