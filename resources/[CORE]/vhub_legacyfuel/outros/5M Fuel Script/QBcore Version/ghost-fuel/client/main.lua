...
function SetFuel(vehicle, fuel)
    if type(fuel) == 'number' and fuel >= 0 and fuel <= 100 then
        ...
        if Config.ElectricVehicles[vehname] then
            QBCore.Functions.TriggerCallback('ghost-fuel:server:GetTimeInGarage', function(time)
                if time then
...
CreateThread(function() -- Set target for pumps and blips
    ...
    exports['qb-target']:AddTargetModel(Pumps, {
        options = {{
            event = "ghost-fuel:PickupPump",
            icon = "fas fa-gas-pump",
            label = Lang:t("info.pickup_pump"),
            entity = entity
        }, {
            event = "ghost-fuel:BuyJerrican",
...
RegisterNetEvent("ghost-fuel:PickupPump", function(data)
    if CurrentPump then
        TriggerServerEvent('ghost-fuel:server:DetachRope', PlayerPedId())
        ...
        TriggerServerEvent('ghost-fuel:server:AttachRope', netIdProp, pumpcoords, GetEntityModel(CurrentPump))
...
RegisterNetEvent("ghost-fuel:RefuelVehicle", function(ped, vehicle)
    ...
    while IsFueling or CurrentCost ~= 0 do
        ...
        if IsFueling then
            ...
            if CurrentCost ~= 0 then
                ...
                TriggerServerEvent('ghost-fuel:server:Pay', math.floor(CurrentCost))
                CurrentCost = 0
            end
        end
    end
            ...
            if IsControlPressed(0, Config.ActionKey) and CurrentCapPos then
                ...
                if #(GetEntityCoords(playerPed) - CurrentCapPos) < 3.0 then -- Mount/Dismount pump
                    if not IsMounted then
                        ...
                        IsFueling = true
                        CurrentCost = 0
                        TriggerEvent("ghost-fuel:RefuelVehicle", playerPed, CurrentVehicle)
                    else
                        -- Dismount
                        if CurrentPump ~= "can" then
                            DetachEntity(CurrentPumpProp, true, true)
    ...
    while true do
        ...
        if CurrentPump and not IsMounted and CurrentPump ~= "can" then       --Check max distance
            if #(GetEntityCoords(ped) - GetEntityCoords(CurrentPump)) >= Config.RopeMaxLength then
                TriggerEvent('ghost-fuel:PickupPump')
...
RegisterNetEvent("ghost-fuel:SetFuel", function(fuel)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    if GetPedInVehicleSeat(vehicle, -1) == ped then
        ApplyFuel(vehicle, fuel)
...
RegisterNetEvent("ghost-fuel:client:AttachRope", function(netIdProp, posPump, model, citizenid)
...
RegisterNetEvent("ghost-fuel:client:DetachRope", function(citizenid, src)
