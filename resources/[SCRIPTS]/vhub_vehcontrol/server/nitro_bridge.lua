...
-- server/nitro_bridge.lua — PONTE de nitro: ficha → vhub_custom (FASE 2 ADR #81)
--
-- O vehcontrol NÃO é dono do nitro. A ficha recebe a intenção do jogador (ligar/desligar,
-- nível, abastecer) e DELEGA ao escritor único `vhub_custom` via seus exports TRUSTED. Toda
...
AddEventHandler(E.NITRO_TOGGLE, function(plate, on)
  ...
  local ok = false
  pcall(function() ok = exports.vhub_custom:nitroSetEnabled(src, p, on == true) == true end)
...
AddEventHandler(E.NITRO_LEVEL, function(plate, level)
  ...
  local p = normPlate(plate); if p == '' then return reply(src, p, false, 'Placa inválida.') end
  local ok = false
  pcall(function() ok = exports.vhub_custom:nitroSetLevel(src, p, level) == true end)
...
AddEventHandler(E.NITRO_CHARGE, function(plate)
  ...
  local p = normPlate(plate); if p == '' then return reply(src, p, false, 'Placa inválida.') end
  local ok = false
  pcall(function() ok = exports.vhub_custom:nitroChargeFromItem(src, p) == true end)
