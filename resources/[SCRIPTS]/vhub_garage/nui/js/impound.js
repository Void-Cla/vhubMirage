  ...
  function renderList() {
    ...
    if (!items.length) {
      $list.innerHTML = `<div style="grid-column:1/-1; padding:60px 0; text-align:center; color:var(--text-dim2);">
        <i class="fa-solid fa-triangle-exclamation" style="font-size:48px; display:block; margin-bottom:8px;"></i>
        Nenhum ve culo no p tio.
      </div>`;
      return;
    }
    items.forEach((it) => {
      ...
      c.innerHTML = `
        ...
        <div class="right">
          <div class="lance" style="color:var(--danger)">${App.fmtMoney(it.fee)}</div>
    ...
    $list.querySelectorAll('[data-pay]').forEach((btn) => {
      btn.onclick = async () => {
        const r = await App.modal({ title: 'Liberar Ve culo', text: `Pagar para liberar ${btn.dataset.pay}?` });
