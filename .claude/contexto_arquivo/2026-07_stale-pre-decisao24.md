# Bloco arquivado de contexto.md — versão desatualizada (achado em 2026-07-08)

> Este conteúdo era uma DUPLICATA ANTIGA que vivia colada dentro de `contexto.md`
> (linhas 371-822 do arquivo original de 1242 linhas), na frente da versão atual
> das mesmas seções (Ownership por resource externo, Contratos de API, Status das
> sprints, Estado de congelamento). Provavelmente sobrou de um `contexto.md` que foi
> atualizado por append em vez de substituição em algum commit passado.
>
> Reflete o estado PRÉ decisão #24 (funil de telemetria física) e ainda cita
> `vhub_login` com a arquitetura antiga de `vhub_vehcontrol` (vEnter/vLeave removidos).
> A versão vigente (mantida em `contexto.md`) já reflete a reorg pós-#24.
>
> **AÇÃO RECOMENDADA**: revisar rapidamente se algo aqui NÃO está no `contexto.md`
> atual e ainda é verdade — se sim, mover para lá. Caso contrário, este arquivo pode
> ser deletado. Mantido por segurança em vez de apagar direto (L-15 aplicado com cautela
> em conteúdo institucional, não em código).
