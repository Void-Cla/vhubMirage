...
_Escritor: `vhub_guardiao_revisao` (escrita de 2026-07-04 AUTORIZADA diretamente pelo DONO — gate do sprint YouTube/DVD) | Atualizado: 2026-07-08 (**#57 — `vhub_money` adota `vhub_target` p/ banco/ATM: gatilho de interação [E]-por-proximidade → OLHO. PRIMEIRO CONSUMIDOR REAL do `vhub_target` (#56).** `client/zones.lua` REESCRITO: 2 threads permanentes/player REMOVIDAS (fria `Wait(700)` + quente `Wait(0)` com `draw_hint` [E] + `DrawMarker` do ATM) + `/banco` APOSENTADO. Novo: no boot registra 1x via `exports.vhub_target:addBoxZone` **77 box zones** — 70 ATMs iterando `VHubMoneyATMs` (icon `card`) + 7 agências `VHubMoneyBanks` (icon `bank`); `canInteract = onFoot` (não abre de dentro do veículo — bloqueio client leve, NÃO verdade crítica); `onSelect → TriggerServerEvent('vhub_money:nui:open',{mode})`. **Opção A do arquiteto (2ª rodada, fato novo):** client usa a MESMA lista curada que o server já revalida em `near_atm`/`near_bank` (`server/init.lua`: raio 3.0/4.0) — NÃO `addModel` de props. `VHubMoneyATMs` = 1 fonte, 3 consumidores (autoridade server-side de validação + blips opcionais + box zones client); zero 2ª fonte de verdade (L-04 OK). **CONTRATO SERVER INTOCADO** — client é só gatilho de UI, server dispõe; gate de segurança dispensado pelo arquiteto por isso (segurança neutra). BLIPS mantidos (não são interação). `shared/config.lua`: `CMD_OPEN_PANEL`/`KEY_OPEN_BLOCK` removidos (órfãos). `fxmanifest.lua`: `dependency 'vhub_target'` (load order OK — target linha 19 antes de money 28 em `resources.cfg`, pré-existente). `web/js/icons.js` do target: SVG `bank`/`card`/`cash` + aliases fa-* (university/credit-card/coins/…) aditivos p/ consumidores futuros. **GANHO PERF:** 2 threads permanentes/player → 0; custo de interação move p/ o scan do target (O(77) `contains` só com olho ABERTO; olho fechado = 0). **Ciclo de vida das zonas:** `addBoxZone` tagueia `data.resource='vhub_money'`; `onClientResourceStop` do target (api.lua:383) as remove ao parar o money → sem órfão (L-07/L-15). **DÍVIDA/RESIDUAL:** `ATM.INTERACT_RADIUS`(1.8)/`BANK.INTERACT_RADIUS`(2.2) em `config.lua` ficaram ÓRFÃOS (client não lê mais; server hardcoda 3.0/4.0) — limpeza incompleta, remover em passada futura (dead config, não bloqueia). **Validação:** `luac -p` OK nos 3 lua do money + `node --check` OK no icons.js (reportado pela tarefa). **CORE FROZEN:** não tocado (só `[SCRIPTS]`, sem schema). **⚠️ ESCOPO — NÃO faz parte do #57:** o diff em `config/resources.cfg` é `+ensure vhub_coinshop` (resource UNTRACKED, alheio ao #57); NÃO comitar junto do #57 — stage seletivo. **TESTES RUNTIME FALTANTES (owner do fluxo confirma in-game):** olho abre painel de ATM e de agência; recusa de dentro do carro (`canInteract=onFoot`); blips intactos; painel abre só dentro do raio server (fora → server recusa). **Rollback:** `git checkout HEAD -- "resources/[SCRIPTS]/vhub_money/client/zones.lua" "resources/[SCRIPTS]/vhub_money/shared/config.lua" "resources/[SCRIPTS]/vhub_money/fxmanifest.lua" "resources/[SCRIPTS]/vhub_target/web/js/icons.js"` + remover `dependency 'vhub_target'`. Gate arquiteto APROVADO (2 rodadas). **próximo livre = #58**) | Atualizado: 2026-07-05 (**#56 — `vhub_target` (`[SCRIPTS]`) CONVERTIDO de ox_target 1.18.1 puro e QUEBRADO (dependia de `ox_lib` inexistente no repo) para o padrao vHub — resource client-side de targeting/UI de mira. SO `[SCRIPTS]`, ZERO CORE.** Contract-compatible com ox_target 1.18.x (nomes de export IDENTICOS = contrato de facto por decisao do arquiteto; quebrar = breaking change com deprecation path + migration, R15). **Deletado (L-15):** `client/framework/{esx,nd,ox,qbx}.lua`, `client/compat/qtarget.lua`, 24 `locales/*.json`, `provide 'qtarget'`, version-check HTTP. **Criado:** `shared/config.lua` (cfg+lang PT-BR global, sem `return` — namespace `VHubTarget`), `shared/events.lua` (R9 — `SET_ENTITY_HAS_OPTIONS`/`TOGGLE_ENTITY_DOOR`/`DEBUG`), `client/init.lua` (cache ped/veiculo 2Hz `Wait(500)` + `_alive` guard no `onResourceStop`; keybind `RegisterKeyMapping` nativo), `client/zones.lua` (**engine propria box/sphere/poly** substitui `lib.zones`; `getNearby()` so roda com olho aberto no tick do loop principal 10-20Hz, O(n)), `web/js/icons.js` (22 SVG locais + aliases `fa-*` — mata FontAwesome CDN). **Server hardening zero-trust** (`server/main.lua`): rate-limit O(1) por `(src,chave)` + cleanup `playerDropped`/`onResourceStop`; `toggleEntityDoor` shape->rate 300ms->`GetEntityType==2`->door 0..5->proximidade 5.0m ao centro->delega ao `NetworkGetEntityOwner` (**nunca escreve entidade** — L-16 OK, sem spawn-write); `setEntityHasOptions` idempotente (R14) + cap 2048 anti-spam + `state:set(...,true)` replicated; GC 10s por HANDLE numerico (nao `__data`) com `_alive` guard (L-18/R8). Zero broadcast `-1`. **A-10:** FontAwesome+Google Fonts CDN REMOVIDOS, scripts classicos (sem ES modules), identidade Areia+Dourado, `textContent` anti-XSS. Filtro `groups` via `vhub_groups:getGroupsLocal` (UX); filtro `items` = **stub permissivo DOCUMENTADO** (validacao real e server-side no consumidor, nao no target client). Exports de registro (`api.*`) tagueiam por `GetInvokingResource()` (ownership/namespacing, L-07) — NAO sensiveis (nao escrevem verdade), por isso `_invoker_allowed()` fica no boundary server (door/flag). **Validacao:** 11/11 Lua `luac -p` OK, 4/4 JS `node --check` OK, 11/11 Lua + 6/6 web no manifest. **DIVIDA/RISCOS ATIVOS:** (1) `client/debug.lua:15` tem `print(json.encode(...))` — TOLERADO: dentro de handler que hard-`return` sem `cfg.debug` + modulo inteiro curto-circuita quando debug=off; dev-only, client `[SCRIPTS]` sem `vHub.Logger`; (2) IIFE web = divida arquitetural aceita, mesmo padrao da #54; (3) hook L-15 tem falso-positivo Windows conhecido com path `[SCRIPTS]` (manifest confere na mao OK). **PENDENTE (nao-DoD):** `ensure vhub_target` no `config/resources.cfg` (nao adicionado — passo do dono/enable). **TESTES RUNTIME FALTANTES (antes de producao):** GC local via `AddStateBagChangeHandler('hasTargetOptions', nil, ...)` (bagName `entity:<netId>` — so in-game); propagacao real do `state:set` replicated ao client; olho abre/fecha + opcoes aparecem + porta de veiculo (owner e nao-owner) via proximidade. **CORE FROZEN:** nao tocado. **Rollback:** arquivos novos untracked (`shared/`, `client/init.lua`, `client/zones.lua`, `web/js/icons.js`) removidos manualmente; tracked via `git checkout HEAD -- resources/[SCRIPTS]/vhub_target/` (restaura o ox_target quebrado — so se regressao). Gates APROVADOS: arquiteto (10 ajustes incorporados), seguranca, natives (2 bloqueantes corrigidos), performance (4 corrigidos), designer, runtime. **proximo livre = #57**) | Atualizado: 2026-07-04 (**#55 — sprint remake visual "prime" SOMENTE CSS (2 NUIs: `vhub_custom/web` Oficina/bennys/mec + `vhub_vehcontrol/html`)**. Refino cosmético, não recriação. A-09: últimas violações `backdrop-filter` sobre o jogo REMOVIDAS de `vhub_custom/web/style.css`+`mec.css` (vidro simulado em camadas, piso ≈0.86–0.90). PADRÃO REUTILIZÁVEL cravado: guard de idle `.<root>.hidden * { animation-play-state: paused !important; }` em TODO root que esconde por `opacity:0` (não `display:none`) → resmon idle 0 com animação `infinite`; aplicado em `.mod-oficina.overlay.hidden`, `.vc-panel.hidden` e `.mod-mec.mec-overlay.hidden` (este ADICIONADO no gate final — gap do achado #1, `vhub_custom/web` é documento único e o guard do oficina não alcançava o root irmão mec). Fonte de display Google Fonts CDN→Bahnschrift/DIN Condensed de sistema (DECISÃO DO DONO; A-10 OK, manifests intactos, zero asset novo). 20 keyframes escopados `mo-/bn-/mec-/vc-`, estados JS preservados, ZERO toque em JS/Lua/HTML/evento/export/schema/CORE. ⚠️ working tree comingled com o sprint #53/#54 não-commitado → commit CSS SELETIVO por arquivo, não `git add -A`. Rollback POR-ARQUIVO (`git checkout HEAD -- <arquivo.css>`; a rollback por-pasta reverteria #53/#54). TESTES RUNTIME: idle 0 em resmon c/ NUI fechada nas 2 UIs (mec com barra `mc-health-low`) + legibilidade do vidro sobre cena clara/escura. **próximo livre = #56**) | Atualizado: 2026-07-04 (**#54 — `vhub_custom` sprint de correção (bennys + mec): bug fixes e UX**. `client/mec.lua` reescrito — `activeVeh` capturado ANTES do `Wait(3000)` (fecha race), `closeMec`+`SendNUIMessage` ANTES da thread de anim, replay-guard `_repairActive` liberado em TODOS os caminhos, helper `awaitControl(ent,timeout)` (NetworkRequestControlOfEntity + loop c/ timeout) reusado no reparo engine/body e no reboque `MEC_TOW_DO` (pneu não precisa — `CVehicleTyreStateSync` propaga sem owner); `server/mec.lua` health `nil`→erro EXPLÍCITO em vez de mascarar como "sem dano" (L-03; guarda defensiva — `conce:getVehicleState` hoje normaliza p/ 1000.0, contrato-futuro-safe); `server/bennys.lua` `calcCost(custPatch)` deriva do patch JÁ validado por `buildCosmeticPatch` (não cobra mods rejeitados pela whitelist; toggles falsy `smoke/xenon=false` cobrados via `~= nil`); `web/mec.js` preço real + barras de saúde motor/lataria; `web/mec.css` `backdrop-filter` REMOVIDO do `.mec-card` (A-09; liquid glass em camadas piso ~0.88) + classes health; `web/bennys.js` `pushPreview` via `_previewRAF` (A-08 coalesce drag do picker) + `_stageMoveRef/_stageUpRef` removidos no `closeNUI` (A-07); `client/zones.lua` thread quente `Wait(16)` budget renegociado no header (L-18). Grep L-13/L-16 limpo. `bindStage()` 1x no boot do IIFE (sem listener duplo). A-10 OK. DÍVIDA: hook L-15 falso-positivo em Windows por path `[SCRIPTS]`; A-02/A-06 do bennys.js/mec.js = IIFE clássico (dívida arquitetural PRÉ-EXISTENTE do resource, não introduzida aqui). Sem toque no CORE. Rollback: `git checkout HEAD -- resources/[SCRIPTS]/vhub_custom/`. **próximo livre = #55**) | Atualizado: 2026-07-04 (**#53 — FRONTEIRA POR MODALIDADE: vídeo sai do `vhub_wow`, vai pro dono da NUI `vhub_vehcontrol`** (telinha "DVD do carro" `#vc-dvd` fora do painel, iframe YT mute — áudio vem do wow; SUPERA a "surface de vídeo" que a #52 pusera no wow: exports `PlayVideoAt*`/`AttachInCarVideo` REMOVIDOS, `PlayAtCoords` áudio-em-coord entra export-first); dupla confirmação `hasVehicleAccess`+`videoId` server-derived; R9 `VIDEO_ATTACH/DETACH` em `shared/events.lua`; 4 guardiões REPROVARAM→corrigido (perf race+pairs(), designer viz-pause, contrato R9, runtime `_ytQueue.length=0`); Google Fonts CDN removido do vehcontrol (paga dívida A-10 da #52); RISCO ATIVO: iframe_api CDN = exceção A-10 consciente (requer ADR própria); `vhub_vehcontrol`→v1.2.0; **próximo livre = #54**) | Atualizado: 2026-07-03 (**#52 — `vhub_wow` v1.1.0: troca Jamendo→YouTube** (InnerTube primário sem chave + APIv3 fallback via convar `wow_youtube_key`; embed nocookie; anti-SSRF `acceptSearchInput`) + **surface de vídeo "DVD no carro" com dupla confirmação** (server propõe attach, client valida `IsPedInVehicle`) + exports de vídeo export-first (gated) p/ "TV da cidade" futura; consumidor `vhub_vehcontrol` recebe botão vídeo + fix pcall no rádio; RISCO ATIVO: scripts embed remotos YT/SC = exceção A-10 consciente + dívida pré-existente Google Fonts no vehcontrol; **próximo livre = #53**) | Atualizado: 2026-07-03 (**#51 — RECONCILIAÇÃO DOCUMENTAL de 3 planos externos com a arquitetura real** (housekeeping sem código de runtime): `PLANO_IMPLEMENTACAO_VEICULOS.md` + `PROMPT_VHUBMIRAGE_CORE_AAA.md` arquivados SUPERSEDED apontando p/ fontes canônicas; `frozen_core_2.md` ganhou 2 notas de reconciliação de numeração (WORKLOG prevalece); skill nova `plano_drift_reconciliacao.md`; **próximo livre = #52**) | Atualizado: 2026-07-03 (**#37–#50 — DESCONGELAMENTO DO CORE v1.0→v2.0.0-alpha.1: FASE 1 do frozen_core_2.md APLICADA e validada em dev** + fixes de runtime + itens estáticos das FASES 2..6 + limpeza do projeto + config aplicada; detalhe completo em `plano_core_v2/WORKLOG_CORE_V2.md`; a numeração de ADRs do worklog CONTINUA esta sequência de decisões) | Atualizado: 2026-06-22 (#34 — `vhub_wow` motor de áudio 3D standalone APROVADO: porta xsound, escritor único de execução de áudio via exports PlayAtEntity/Destroy/SetVolume; soundName derivado server-side fecha spoof; netId gated por hasVehicleAccess; pasta referencia/ removida; vhub_voicePMA negado como expansão, nasce separado) | Atualizado: 2026-06-22 (#33 — CONSULTIVA/REJEITADA: proposta externa `vhvehicle` centralizador para substituir `vhub_conce` REPROVADA pelo arquiteto — premissa falsa, viola L-14/L-04/Doutrina da Placa; vh.md deletado, zero código mudou) | Atualizado: 2026-06-17 (fix #26 — persistência de tuning: customization MERGE-não-REPLACE por chave/índice em `VState:save`; turbo exclusivo da oficina via campo booleano; oficina persiste GTA-level; `/tuning` admin = visual-only) | Atualizado: 2026-06-15 (decisão #25 — ownership da config de ZONA de veículo movida p/ os donos de negócio: concessionária→`vhub_conce`, leilão→`vhub_ferinha`; garage = consumidor via PULL `getZones` no boot; lei nova L-19 vec3/vec4 + fronteira flat) | Atualizado: 2026-06-19 (#27 engine de skill em vhub_vehcontrol + #28 F5 física do skill confirmada em jogo + #29 vhub_nitro reescrito vRP→vHub; **DOUTRINA DA PLACA** cravada: toda verdade veicular persistida mora na placa via conce, derivados nunca persistem) | Atualizado: 2026-06-19 (#30 — nitro vira EIXO da FICHA do veículo: uso por proximidade APOSENTADO; liga/desliga + nível 1..10 (trade-off durabilidade↔velocidade) + Abastecer pela ficha do vhub_vehcontrol; vhub_nitro segue escritor ÚNICO de customization.nitro={kit,qty,enabled,level}, vehcontrol DELEGA via 3 exports TRUSTED + bridge) | Atualizado: 2026-06-20 (#31 — vhub_notify = toast global PADRÃO vHub: canal canônico evento `vHub:notify` distinto de vHub.Notify/webhook; reescrita QDEV/vule→Mirage liquid glass; textContent/rate-limit/clamp; legacyfuel já consome; resource UNTRACKED exige git add) | Atualizado: 2026-06-21 (#32 — sprint hardening segurança: N0-1 lock+compensação ACT_TRANSFER, transferOwner atômico; N0-2 exports default-deny; N0-3 handlers veiculares do CORE desarmados; core-frozen-v2.0) | Modelo: Claude Opus 4.8_
...
| **vhub_custom** | `[SCRIPTS]/vhub_custom/*` | **Oficina (F0-F4)**: bennys (estética `source='cosmetic'`), mec (reparo `source='repair'` + reboque), oficina (tuning `source='tune'`). Sem schema SQL próprio. Escritor de `vhub_vehicle_state` VIA `exports.vhub_conce:saveVehicleState`. `canOperate` obrigatório antes de todo save. NUI (bennys/mec/oficina) CONSTRUÍDA — sprint de correção #54 (bug fixes + UX). |
| **vhub_target** | `[SCRIPTS]/vhub_target/*` | **Targeting / UI de mira ("olho")** — L2/L3 client-side puro. **SEM persistencia, SEM verdade critica.** Owner do registro de opcoes de interacao (globais/por-modelo/por-netid/por-entidade-local, tagueadas por `GetInvokingResource()`) e da engine propria de zonas box/sphere/poly (`client/zones.lua`, substitui `lib.zones`). Contrato = **exports client contract-compatible com ox_target 1.18.x** (nomes identicos = contrato de facto; quebrar exige deprecation path + migration, R15) — registro namespaced por invoker, NAO sensivel. Hardening real mora em `server/main.lua`: validacao zero-trust `SET_ENTITY_HAS_OPTIONS`/`TOGGLE_ENTITY_DOOR` (shape->rate->entidade->tipo->alcance) + delegacao de porta ao `NetworkGetEntityOwner` (nunca escreve entidade — L-16 OK). Consumidores: **`vhub_money` (banco/ATM, #57 — PRIMEIRO consumidor real)** via `addBoxZone` (box zones registradas 1x no boot, `data.resource='vhub_money'` → `onClientResourceStop` limpa; L-07/L-15 OK). |
...
| Export cross-resource sem controle | `_invoker_allowed()` + `cfg.trusted_resources` whitelist. **N0-2 (decisão #32): default-DENY** — trust vazio/`nil` ou `caller==nil` ⇒ `false`; negação por trust-vazio loga warn one-shot. **#41/#49 (2026-07-03):** trust agora é populado pelo convar `vhub_trusted_resources` no `config/server.cfg` (9 resources: conce/garage/custom/vehcontrol/nitro/racha/admin/ferinha/legacyfuel); a superfície gated cresceu (#39: `commitVehicleState`/`registerVehicleSpawn`/`registerVehicleDespawn`) e existe caller externo REAL (conce espelha fuel, #44) — o blast radius do trust deixou de ser nulo: convar ausente ⇒ espelho de fuel e commits NEGAM (warn de boot orienta o fix, F-022) |
...
| Ownership de entidade errado | ~~handlers desarmados (N0-3/#32)~~ **SUPERSEDIDO por #37/#47 (descongelamento, 2026-07-02)**: `vEnter/vLeave/vState` REANIMADOS com gate FÍSICO server-side — só aceita se `GetPedInVehicleSeat(ent, seat) == GetPlayerPed(src)` E a placa REAL da entidade == a alegada (normalizadas, espaços colapsados). Forjar vEnter com netid da vítima FALHA: o atacante não está sentado nela. `vLeave` só de quem tem `occupants[src]` registrado por vEnter validado. `vSpawned/vDespawned` seguem NET-desarmados (spawn é verdade server-side via exports gated `registerVehicleSpawn/Despawn`). **`NetworkSetEntityOwner` REMOVIDO (#47): a native NÃO EXISTE no build server (crashava — pego em dev 2026-07-02); OneSync migra ownership ao motorista sozinho; a autoridade do CORE é `vd.driver` (gate de quem reporta vState)**. Kill-switch: `GlobalState.vh_core_active=false` desarma a emissão client sem restart. Pentest de spoof (PARTE V) pendente antes de produção |
...
| ~~`onStateUpdate` dormente~~ **REANIMADO (#37; validado em dev 2026-07-03)** | `client/vehicle.lua` v2 emite vEnter/vLeave (transição de assento estável por 2 passadas ≈ réplica propagada + reafirmação idempotente a cada 30s, R14) e vState adaptativo 0.5–4Hz — TUDO gated por `GlobalState.vh_core_active` (F-019 fechado: antes 100% do tráfego morria num no-op). O CORE processa o report validado (`vd.driver==src`) e escreve as bags `vh_odo/vh_eng/vh_body/vh_on` (consumidores reais: vhub_velo/vehcontrol). **Fuel segue em SHADOW (#38)**: sem drain e sem `vh_fuel` (convars `vhub_core_fuel`/`vhub_veh_state_apply` default 0) — o PRONTUÁRIO do conce permanece a fonte persistida ATIVA e o vehcontrol o aplicador físico até o cutover da FASE 2 (sem escritor duplo, L-04) |
...
| **Gate de login fail-OPEN contra ausência do selector** (decisão #36) | `vhub_login` só intercepta o spawn quando `vhub_player_state` emite `chooseSpawn`, o que exige `GetResourceState('vhub_spawselector')=='started'` (player_state server.lua:149). Selector parado + `login.enabled=true` ⇒ player spawna no mundo SEM credencial. **MITIGAÇÃO PENDENTE** (não-bloqueante hoje pois `enabled=false`): `ensure vhub_spawselector` + monitor, OU (preferível) player_state emitir hold/`chooseSpawn` incondicional quando o gate de login está ativo. Resolver/documentar no runbook ANTES de `enabled=true` |
| Op SQL envenenada no batch (ex.: FK violada) re-enfileirava PARA SEMPRE — flood de log+DB visto em dev 2026-07-02 com `/spawncar` (placa sem âncora) | **#47 (defesa em DUAS camadas no CORE)**: (a) cap `MAX_OP_RETRIES=5` em `state.lua:_flush` — op descartada com ERROR após 5 tentativas (vale p/ QUALQUER falha futura, não só FK); (b) placa SEM âncora em `vh_vehicles` ⇒ `Veh:register` marca `vd.anchored=false` = registro EFÊMERO (VRAM-only + bags, ZERO SQL) — o CORE NÃO cria âncora (escritor da identidade: conce, L-04). Complementa o gate client-side da #22c. **Débito**: placa que ganha âncora DEPOIS do registro fica efêmera até re-registro (restart/unregister) — revisar na FASE 2 |
| VRAM do CORE crescia linear (ban.active/whitelist/datatable de todo uid que já conectou, residentes p/ sempre — F-013) | **#40**: eviction por TTL de entidade — `_touch[et][eid]` atualizado em `S:get/S:set` (O(1)); varredura a cada 60s remove buckets ociosos >1h inteiros (yield a cada 200). Online NUNCA evicta (autosave 60s toca `ud` da sessão); miss pós-eviction = re-read do DB (caminho que já existia). Sem exceção de hot-keys: ban/whitelist só são lidos no connect — residência permanente era desperdício, não otimização |
...
37. **DESCONGELAMENTO DO CORE — FASE 1 aplicada e VALIDADA EM DEV + fixes de runtime + itens estáticos das FASES 2..6 + limpeza (#37–#50, 2026-07-02/03; escrita neste arquivo AUTORIZADA explicitamente pelo dono nesta sessão).** Registro DETALHADO (mapa arquitetural do CORE linha-a-linha, métricas antes/depois, checklists, rollbacks por item) em **`.claude/plano_core_v2/WORKLOG_CORE_V2.md`** — este bloco é o resumo institucional; o worklog é a fonte completa. A numeração de ADR do worklog CONTINUA esta sequência (**#37..#50 são os MESMOS números lá; próximo livre = #51 nos dois arquivos**). CORE `1.0.0` → **`2.0.0-alpha.1`** (banner FROZEN atualizado p/ status gated). Rollback global: tag git `pre-frozen-core-2-remediation` (aponta `bde7784`; CORE estava limpo). Gate formal: fase atual do CLAUDE.md (descongelamento) + `settings.local.json` allow de sprint; agentes nativos não carregáveis na sessão (aberta um nível acima do repo) → gates executados como passes internos contra os checklists dos guardiões, registrado aqui por honestidade.
    - **#37 — Reanimação GATED de `vEnter/vLeave/vState`** (supersede o "NUNCA reanimar" de #24/#32 SOB o novo gate que a própria #24 previa): validação FÍSICA contra a réplica server-side (`GetPedInVehicleSeat(ent,seat)==GetPlayerPed(src)` + placa real==alegada) fecha o vetor original do ADR #24 na raiz. `vSpawned/vDespawned` seguem NET-desarmados — spawn entra por exports gated `registerVehicleSpawn/registerVehicleDespawn` (donos: garage/conce). Kill-switch `GlobalState.vh_core_active` (client só emite quando true; F-019). Client v2 (`client/vehicle.lua` reescrito): transição de assento estável 2 passadas, reafirmação idempotente 30s (R14), vState só do driver, loop adaptativo 0.5–4Hz inalterado. Handler `vHub:passengerMode` registrado no client (F-007).
    - **#38 — SHADOW-MODE de fuel**: `cfg.core_fuel_enabled=false` (drain OFF, `vh_fuel` bag OFF) e `cfg.veh_state_apply=false` (`vehicleStateLoad` não emitido) até o cutover da FASE 2 — evidência: `vhub_velo` já lê `vh_fuel` com fallback native; CORE escrevendo bag com default 100.0 divergiria do tanque real, e o vehcontrol já é o aplicador físico (requestState). Bags REATIVADAS na FASE 1: `vh_odo/vh_eng/vh_body/vh_on/vh_tune` (dados validados do report).
    - **#39 — Contrato oficial de commit no CORE**: `commitVehicleState(plate, patch, source)` com SOURCE_GATES por origem (pump→fuel; repair→health/damage; tune→tuning; garage→lifecycle; system→tudo), gated `_invoker_allowed`, VRAM+bags+SQL batch+`vHub:vehicleCommitted` (constante `vHub.E.EVT_VEH_COMMITTED`, resolve F-024 na origem)+audit. `getVehicleState(plate)` devolve CÓPIA (L-14). NÃO migra o conce nesta fase — prontuário segue fonte ativa (coexistência transitional documentada; FASE 2 = cutover).
    - **#40 — VRAM eviction por TTL de entidade** (F-013): detalhe na tabela de riscos acima.
    - **#41 — Config unificada** (F-003/004/005/017/022): `criar_config()` agora termina em `vHub.mergeConfig` (defaults entram por baixo dos convars — `max_ping/veh_state_hz/max_speed_kmh/lang.*` deixaram de ser nil); mergeConfig ganhou merge de 1 nível p/ tabelas parciais; `_defaults` alinhados ao efetivo (fuel_rate 0.005, max_speed 350 — zero mudança de comportamento); `validateConfig` FAIL-FAST no boot; convar `vhub_trusted_resources` (CSV→set) popula o trust sem editar código; warn de boot com lista sugerida quando vazio.
    - **#42 — Audit unificado `vh_audit`** (F-073 parcial): tabela append-only no schema do CORE (nome `vh_` por convenção, diverge do `vhub_audit_unified` do plano — deliberado); `vHub.audit(actor,action,target,source,before,after)` em `state.lua` → op no batch (zero custo síncrono); wired em commitVehicleState + grantPerm/transferKey/banPlayer/unbanPlayer (R12). Trigger SQL de imutabilidade = FASE 4.6 (exige privilégio TRIGGER).
    - **#43 — Bump `2.0.0-alpha.1`** (exigência do gate do CORE). Alpha até a FASE 8 validar runtime.
    - **#44 — Dual-write de FUEL conce→CORE (FASE 2.1-lite)**: após save com `patch.fuel` no prontuário, `VState:save` espelha `{fuel}` via `exports.vhub:commitVehicleState(p,{fuel},'pump')` em thread própria (camada B, soft-dep pcall+GetResourceState). SÓ fuel — health/odômetro o CORE já recebe da telemetria validada; espelhar seria 2º escritor (L-04). Aquece o vd.state com dado REAL p/ o cutover.
    - **#45 — Hooks v2 promovidos**: as versões v2 dos hooks de enforcement (L-13/14/15/16/17 + SQL/git destrutivo) viviam na RAIZ mas `settings.json` executa `.claude/hooks/` que tinha as v1 fracas — **o enforcement v2 NUNCA rodou**. v2 copiada p/ `.claude/hooks/`; cópias da raiz removidas (fonte única).
    - **#46 — Vetores de abuso fechados por config**: conce `test_drive_segundos 9999→300` + `fator_test_drive 0.00→0.10` (F-027); vehcontrol `skillDebug true→false` (F-051) e `skillBruteTest true→false` (**anti-P2W estava DESLIGADO em produção**, F-052).
    - **#47 — Fixes de RUNTIME (dev 2026-07-02 expôs 3 falhas; boot 2026-07-03 LIMPO)**: (a) `NetworkSetEntityOwner` NÃO existe no build server — removido de onEnter/onLeave (era código pré-freeze jamais executado; OneSync migra ownership sozinho; autoridade = `vd.driver`); (b) registro EFÊMERO p/ placa sem âncora (ver tabela de riscos); (c) cap de 5 retries no batch (poison-op não loopa mais). Validação: `/spawncar sultan2` + entrar/dirigir sem 1 erro no log.
    - **#48 — Itens estáticos das FASES 3/4/5/6**: (a) F-043 ELIMINADO — os 6 broadcasts `TriggerClientEvent(-1)` de despawn (impound×2/rental/admin×2/conce) viraram `Core.despawnByPlate` server-side (`GetAllVehicles`+`DeleteEntity`, L-05; zero rede; handler client `DO_DESPAWN` mantido por compat; `DO_REPAIR -1` permanece — reparo usa natives client-only); (b) F-045 — export `impoundVehicle` do garage GATED (`IMPOUND_TRUSTED` default-deny + log `impound_denied`; `vhub_garage` na lista pois `forceImpound` delega via exports); (c) `_invoker_allowed` do garage flipado p/ default-deny em caller nil — NOTA DE HONESTIDADE: caller nil = chamada interna (idioma padrão FiveM, NÃO é vuln de cliente); flip seguro AQUI porque os 3 exports são cross-resource-only (verificado); o MESMO idioma em conce/money/ferinha/inventory/ipad/groups NÃO foi flipado às cegas (quebraria chamadas internas legítimas) — auditar caso a caso na FASE 4; (d) F-031 — conce chama export gated `despawnVehicleByPlate` novo em vez de evento hardcoded do garage (conce entrou no TRUSTED do garage); (e) F-029 — `reconcileOrphans()` no boot do PRÓPRIO conce (pcall, pós-backfills); (f) F-058 — Drift ganhou `onResourceStop`→`revertDrift` (deltas simétricos; carro não fica com handling viciado pós-restart); (g) FASE 3.1/3.2 — exports `getVehicleDriver`/`getVehicleOccupants` no CORE (cópias); (h) F-057 STALE (wow já envia targeted); F-038/F-039 STALE (já resolvidos no código); F-055 ADIADO (listener seria código morto até handling fluir pelo CORE — L-15). **F-059 NÃO tocado de propósito**: conflito REAL confirmado (Drift e vehcontrol mutam `fTractionCurveMax/Min` — `Drift/cl.lua:46-47`), mas separar campos muda FÍSICA de gameplay → decisão exige teste em jogo (FASE 6).
    - **#49 — Config aplicada + resources.cfg por camada**: `setr vhub_trusted_resources "vhub_conce,vhub_garage,vhub_custom,vhub_vehcontrol,vhub_nitro,vhub_racha,vhub_admin,vhub_ferinha,vhub_legacyfuel"` no `config/server.cfg` (legacyfuel preventivo — futuro escritor de fuel via commit, export-first); `config/resources.cfg` reescrito em 10 camadas comentadas (infra→kernel→identidade→economia→personagem→veículos→corrida→policial→admin→periféricos→mundo) — 34 ensures preservados (diff `comm` = zero perda/adição); comentário stale corrigido (ordem de boot real = `dependency` dos fxmanifests, não a lista).
    - **#50 — `getVehicle` do CORE devolve CÓPIA** (`dataCopy`): o GATILHO da #32 exigia fechar o "VD vivo por referência" ANTES de re-armar a cadeia física; a reanimação (#37) reativou o risco residual. Grep 2026-07-03: zero consumidor direto em [SCRIPTS] → cópia segura por construção. `getVHub()` permanece por contrato (#36 login/oxmysql dependem) — L-14 continua valendo p/ quem o usa.
    - **LIMPEZA do projeto (mandato do dono, "deletar é entrega" L-15)**: removidos `.claude/core no frozen/` (duplicata byte-a-byte de `plano_core_v2/`), `.claude_backup_remaster_20260628_091350/`, `.claude/_arquivo/` (hooks .v1), hooks da raiz (promovidos, #45), `tmp_ctx_split.py`, `SCRIPTS.zip` (604K), `ox_doorlock-main/` + `snowy_characterselector-master/` (terceiros baixados, zero ensure/referência), 9 handlers `100fuel..0fuel` do legacyfuel (F-040 — zero emitter; dariam fuel grátis). MANTIDOS deliberadamente: `contexto_arquivo/` (arquivo sob demanda), `metas/*`, `sss.txt`, testplans de `[CAR]`, `GOVERNANCA_DELTA.md`, `vhub_target`/`vhub_voicePMA` (WIP first-party, decisão de matar é do dono), `build/`+`cache/` (runtime FXServer, gitignored). **Doc-drift corrigido no CLAUDE.md** (raiz + cópia `.claude/`, sincronizadas): compat vRP removido do header, L-11 marcada REVOGADA, ordem de load sem `compat`, STATUS da fase atual.
    - **TESTES_FALTANTES (runtime, ANTES de produção — checklist PARTE V do frozen_core_2.md)**: pentest de spoof de vEnter (netid da vítima → DEVE ser rejeitado com warn); 2 players mesmo carro (driver/passageiro; bags corretas); despawn server-side (impound/rental/admin) com carro ocupado e vazio; `vh_audit` populando em commit/ban/grantPerm; espelho de fuel conce→CORE com trust populado; eviction de VRAM em soak 1h+; stress 64 players (budget ≤1.0ms tick); resmon do CORE.

51. **RECONCILIAÇÃO DOCUMENTAL — 3 planos externos alinhados à arquitetura real (2026-07-03; housekeeping SEM código Lua/JS; autonomia-com-gates: veredito `vhub_arquiteto` + gate final `vhub_guardiao_revisao`).** Sprint fechou drift entre 3 docs de planejamento e a arquitetura real. **Nenhum runtime tocado** → NÃO é ADR de código (não vai no WORKLOG); é decisão de memória institucional. Numeração: #51 confirmado LIVRE (WORKLOG só reservava `próximo livre=#51`; contexto e Apêndice B do frozen idem — zero colisão). Próximo livre = #52.
    - **`frozen_core_2.md` MANTIDO (só 2 notas, ZERO renumeração — rastreabilidade de auditoria preservada)**: (a) header pós-"Princípio norteador" avisa que a numeração F-XXX/ADR do Apêndice B é PROPOSTA; execução real vive no `WORKLOG_CORE_V2.md` (#37..#50 aplicadas) e **em divergência o WORKLOG prevalece**; (b) topo do Apêndice B explicita que ele propõe #36..#55 mas o WORKLOG aplicou #37..#50 com semântica parcialmente diferente (ex.: aqui #37=`commitVehicleState`; no WORKLOG #37=reanimação gated de handlers). ADR #51 lá é PROPOSTA (Risco #1 handling), não aplicada.
    - **`PLANO_IMPLEMENTACAO_VEICULOS.md` → SUPERSEDED** (corpo ~11KB "Manus AI" trocado por bloco de arquivamento). Drift confirmado contra o código: usava `MySQL.Async.*` (projeto = `exports.oxmysql`); tabela `player_vehicles` + colunas físicas `fMass/fDriveBiasFront/fTractionMax` INEXISTENTES (verdade veicular = `vhub_vehicle_state` no conce, doutrina da placa #24/#32, handling derivado nunca persiste); módulos de handling dentro do `vhub_conce` criariam 2º dono (viola L-04/R4 — dono real = `vhub_vehcontrol/shared/tier_rules.lua::TR.handlingFromAlloc`, já existe); `onResourceStart→SELECT` full-table (P4) + `print()` colorido (R10/L-08). Intenção legítima (clamp físico por arquétipo) preservada apontando p/ `frozen_core_2.md` §F-053/§F-068 + ADR #51 PROPOSTA; ownership do clamp quando aplicado = `vhub_vehcontrol` (conce NÃO ganha módulo de handling).
    - **`.claude/PROMPT_VHUBMIRAGE_CORE_AAA.md` → SUPERSEDED** (corpo ~13KB trocado por bloco de arquivamento). Conteúdo válido (camadas/VRAM/anti-limbo/lazy-load) já vive em `CLAUDE.md` (L-01..L-19, A-01..A-10) e `frozen_core_2.md` (R1..R15, P1..P6) — reescrever = duplicar. Cláusula "autonomia sem pedir permissão" CONFLITA com o modelo real (autonomia-COM-gates, CLAUDE.md 2026-06-27) → não reconciliável. Paths com typo (`vhubMirage.claude/...`), leis com número/semântica trocada. Aponta p/ fontes canônicas.
    - **Skill nova `.claude/skills/plano_drift_reconciliacao.md`**: padrão validado — doc externo que diverge da arquitetura → ARQUIVAR SUPERSEDED apontando p/ fonte canônica em vez de reescrever (reescrever = 2ª fonte de verdade em nível meta, viola L-04/R4 no plano de planejamento). Inclui árvore de decisão (manter/superseded/reescrever/deletar), anatomia do bloco, checklist de drift FiveM, nota de governança (housekeeping documental só exige arquiteto + revisão).
    - **RISCO RESIDUAL (dangling refs, NÃO bloqueia — flag do task #3)**: 2 docs frozen ainda citam `PLANO_IMPLEMENTACAO_VEICULOS.md` como se ativo — `01_reference_docs.md` §3.10 + §4 (a §4 REPRODUZ a arquitetura arquivada do plano, incl. `boot_service.lua` no conce) e `07_handling_balancer.md:1700`. Não quebra leitura: o path resolve (arquivo existe agora com header SUPERSEDED); §3.10 já NEUTRALIZA a abordagem runtime do plano ("script.md supera... 2ª fonte de verdade proibida por L-04"); e a hierarquia de verdade (código→CLAUDE.md→contexto→plano_core_v2) põe esses docs abaixo. NÃO reescritos DE PROPÓSITO: são o corpus de auditoria frozen — reescrever perde rastreabilidade (regra da própria skill). Quem for a §4 do `01_reference_docs.md` deve saber que ela é pedigree, não fonte operacional.

52. **`vhub_wow` v1.1.0 — troca Jamendo→YouTube + surface de vídeo "DVD no carro" (2026-07-03 — GATE FINAL APROVADO; arquiteto aprovou placement + 5 guardiões em paralelo aprovaram o diff real; autonomia concedida, dono ausente). `[SCRIPTS]`, ZERO toque no CORE → sem ADR de CORE.** Evolui o motor de áudio da #34. Bump `1.0.0`→`1.1.0`.
    - **Provider de música trocado: Jamendo REMOVIDO, YouTube ENTRA.** `server/music.lua` (Jamendo) DELETADO; renomeado→`server/music_search.lua` mantendo o contrato histórico `WOW_Music.searchTracks`/`radioTrackSync` (R15 — sem quebra p/ o consumidor). Novo registry com **failover em cascata**: `server/providers/youtube_innertube.lua` (PRIMÁRIO — endpoint interno `youtubei/v1/search`, sem chave/sem cota, chave InnerTube pública embutida) → `server/providers/youtube_apiv3.lua` (FALLBACK — YouTube Data API v3, `ready()` fail-closed exige convar server-only `wow_youtube_key`; inerte enquanto vazia). Ambos: parser DEFENSIVO (pcall + type-guard em cada nível), `MAX_BODY` (4 MB InnerTube / 1 MB APIv3), normalização descarta id≠11 chars e **reconstrói a URL via `buildEmbedUrl` (nunca confia na URL do payload)**. Cache de busca TTL 30 min + teto 64 (anti-leak, L-18); rádio = playlist de `radioSeed` (videoIds semente em config, validados, sem egress). Embed sempre **nocookie** (`www.youtube-nocookie.com`) — menos anúncio/tracking.
    - **Anti-SSRF no buscador (`WOW_Config.acceptSearchInput`)**: recusa qualquer `http(s)://` que NÃO seja YouTube ANTES do egress; aceita só URL-YouTube (toca direto) ou texto de busca 1..80 chars sem controle. `isValidUrl`/`parseYouTubeId`/`isSoundCloudUrl` usam host ANCORADO (`^…$`), não substring.
    - **Surface de vídeo "DVD no carro" (existência ≠ visibilidade).** O áudio já toca; o vídeo é opt-in com **dupla confirmação**: (1) server só PROPÕE o attach (`AttachInCarVideo(target,soundName,netId)`), (2) o CLIENT valida localmente `IsPedInVehicle(ped, ent, false)` antes de exibir o iframe (R1/L-16 — anti "ver DVD do carro alheio"; L-16 spawn NÃO se aplica: é visibilidade de NUI, não spawn de entidade). `soundName` SEMPRE derivado de `src` no consumidor (`vc_radio_%d`) — não forjável. NUI move o iframe já existente entre container oculto↔palco (não cria player novo); label via `textContent` (anti-XSS, nunca innerHTML); palco liquid-glass A-09 (sem `backdrop-filter`, tons quentes Mirage rgba 26,22,16 / 12,10,6, body transparente).
    - **Exports export-first (gated `callAllowed` default-deny + `TrustedResources` fail-closed + rate-limit por resource `_opAt` indexado por nome=sem leak):** `PlayVideoAtEntity`/`PlayVideoAt`/`StopVideoAt`/`AttachInCarVideo`/`DetachInCarVideo`. `PlayVideoAt` (coordenada fixa) SEM consumidor hoje — superfície pronta p/ **"TV da cidade" futura** (outdoor/palco/boteco); legítimo sob a convenção export-first (não é dead-code L-15). `GetRadioTrack`/`RequestSearch` mantidos (busca async via evento `vhub_wow:searchResults`; rádio síncrona do cache — funcref NÃO cruza export cross-resource).
    - **Consumidor `vhub_vehcontrol`** (soft-dep, sem `dependencies{}`; sem vhub_wow o rádio só não funciona): +evento `soundVideo` (attach/detach; exige `VHubVeh.hasVehicleAccess(src,plate)` + tipos validados + tudo em pcall). **FIX de segurança aplicado neste gate:** rádio `PlayAtEntity` agora envolto em pcall (R7 — vhub_wow pode reiniciar entre `wowAvailable()` e o disparo). `_searchAt` (rate-limit busca 1.5s/player) limpo em `playerDropped` (sem leak). NUI: botão `#sound-video`, `el.soundVideo`, toggle vídeo + `resetVideoToggle` no `setPlaying`, `.vc-sound-video` glass/dourado.
    - **CONVAR:** nova `wow_youtube_key` (opcional; vazia = InnerTube-only). REMOVIDA `wow_jamendo_id`. **Ação do dono no `server.cfg`/documentação pendente.** Grep de fechamento: ZERO referência residual a Jamendo/music.lua em todo `[SCRIPTS]`; 8/8 arquivos Lua `luac -p` OK; manifest `files{}` declara html/index.html + audio.js; ordem `providers→music_search→exports` correta.
    - **RISCO ATIVO (backlog, NÃO introduzido por esta sprint):** (a) `vhub_wow/html/index.html` carrega 2 `<script src>` remotos oficiais — `youtube-nocookie.com/iframe_api` e `w.soundcloud.com/player/api.js` — documentados como **exceção consciente ao A-10** (API de embed de terceiro NÃO tem versão local/embarcável; a feature "player YouTube" é intrinsecamente online — sem internet a telinha não monta, e o código degrada suave: `onVideoAttach` faz `return` se backend≠youtube). É a única forma de cumprir a feature; não bloqueia. (b) **DÍVIDA PRÉ-EXISTENTE fora de escopo:** `vhub_vehcontrol/html/index.html` usa Google Fonts CDN (viola A-10) — anterior a esta sprint; registrar p/ correção futura (fonte do sistema/embarcada). (c) camada espacial do áudio segue ANDAIME (sem atenuação real; herdado da #34); YT/SC tocam 2D no CEF. **Próximo livre = #53.** ⚠️ **SUPERADA PARCIALMENTE pela #53:** a "surface de vídeo" descrita acima (exports `PlayVideoAtEntity`/`PlayVideoAt`/`StopVideoAt`/`AttachInCarVideo`/`DetachInCarVideo` no vhub_wow + palco `#wow-video-stage` no html do wow) foi **REVERTIDA** — o vídeo saiu do wow e virou responsabilidade do consumidor de UI (`vhub_vehcontrol`). Ler #52 como "áudio=YouTube" ✅ + "vídeo no wow" ❌ (não existe mais no código; prevalece a #53).

53. **FRONTEIRA POR MODALIDADE — vídeo sai do `vhub_wow`, vai pro dono da NUI (`vhub_vehcontrol`) (2026-07-04 — GATE FINAL APROVADO; 4 guardiões corrigiram/aprovaram o diff real; autonomia concedida, dono ausente). `[SCRIPTS]`, ZERO toque no CORE → sem ADR de CORE.** Reformula a "surface de vídeo" que a #52 tinha colocado NO wow. Motivação: **CEFs são isolados** — dois resources (`vhub_wow` e `vhub_vehcontrol`) NÃO compartilham DOM/JS; um iframe montado no CEF do wow é invisível ao vehcontrol. Regra nova: **áudio = `vhub_wow` (dono do stream/URL, `PlayAtEntity`); vídeo = quem tem a NUI (`vhub_vehcontrol`, telinha "DVD do carro")**. L-04 preservado: áudio e vídeo são DADOS DIFERENTES com DONOS DIFERENTES (não é 2ª fonte da mesma verdade). Bumps: `vhub_wow` `1.1.0` (mantido), `vhub_vehcontrol` `1.1.x`→`1.2.0`.
    - **`vhub_wow` ENXUGADO (removida a surface de vídeo da #52):** `server/exports.lua` — DELETADOS `PlayVideoAtEntity`/`PlayVideoAt`/`StopVideoAt`/`AttachInCarVideo`/`DetachInCarVideo`; ADICIONADO `PlayAtCoords` (áudio 3D em coord FIXA, export-first gated p/ "TV da cidade"/palco/boteco futuro — sem consumidor hoje, superfície pronta; client handler `vhub_wow:playAt`). `html/index.html`/`html/audio.js` — palco de vídeo `#wow-video-stage` REMOVIDO; sobra só o container OCULTO `#wow-players` (motor de áudio headless). `client/engine.lua` v1.1.0 = motor de atenuação 3D (nasce no 1º som posicional, morre no último — L-06 zero idle), agora com netId OU coords. Grep de fechamento: ZERO referência residual a `PlayVideoAt*`/`AttachInCarVideo`/`wow-video-stage`/Jamendo em todo `[SCRIPTS]` (L-15 limpo).
    - **`vhub_vehcontrol` GANHA a telinha (novo dono do vídeo):** `html/video.js` (NOVO, em `files{}` — A-10 p/ o script local) = mini-player DVD do carro. Monta `YT.Player` em MUTE (host nocookie) — o **áudio vem do wow**, o iframe é só imagem. `#vc-dvd` fica **FORA de `#vc-panel`** de propósito: persiste com o menu fechado (música + telinha continuam). Só o `videoId` de **11 chars validado server-side** entra no player (regex `^[\w-]{11}$` espelha o servidor; nunca string crua; `textContent` no título = anti-XSS). Cleanup A-07: `destroyPlayer()` faz `player.destroy()` + `dvdFrame.innerHTML=''` + **`_ytQueue.length=0`** (descarta builds pendentes se a IFrame API ainda não carregou — sem leak de montagem tardia).
    - **DUPLA CONFIRMAÇÃO (R1/L-16-análogo — anti "ver DVD do carro alheio"):** (1) server `soundVideo(netId,plate,show)` exige `VHubVeh.hasVehicleAccess(src,plate)` + só exibe se houver um som **YouTube** ATIVO do próprio player (`_now[src]` + `youTubeIdOf(url)`); (2) o `videoId` deriva da URL do som ativo (server-side), nunca do payload do cliente. `soundName` SEMPRE `vc_radio_%d` derivado de `src` (não forjável). L-16 spawn NÃO se aplica: é visibilidade de NUI, não spawn de entidade.
    - **R9 (nomes de evento centralizados) — CORRIGIDO neste gate:** `VHubVeh.E.VIDEO_ATTACH` (`vhub_vehcontrol:soundVideoAttach`) e `VIDEO_DETACH` (`:soundVideoDetach`) registrados em `shared/events.lua`; server `sound.lua` e client `sound.lua` usam a constante (não literal solto). `_now[src]` (som ativo por player, base do DVD) + `_searchAt` limpos em `playerDropped` (sem leak). Todas as chamadas cross-resource ao wow em pcall (R7).
    - **NUI (aba Som):** checkbox "Vídeo no carro" (`#sound-video`) dispara `soundVideo{show}`; `refreshVideoIfOn()` reexibe a telinha ao trocar de faixa (dá 250ms p/ o server registrar o som antes); `resetVideoToggle()` desmarca o checkbox quando o som para. `soundViz.is-paused` aplicado no `hidePanel` (fix idle A-09 — visualizador não anima com NUI fechada). `style-sound.css`: `.vc-dvd` liquid-glass SEM `backdrop-filter` (A-09), color token `#0e0f14`→`var(--vh-black)`. `style-core.css`: Google Fonts CDN REMOVIDO → fontes de sistema via `--vh-font-*` (A-10 — **paga parte da dívida (b) da #52**).
    - **RESULTADO DOS GUARDIÕES (todos os REPROVADOS foram corrigidos ANTES deste gate):** Performance REPROVOU→corrigido (race condition no `startEngine` = guard `if soundCount>0 then startEngine() end` pós-loop; `toDestroy[]` coletado FORA de `pairs()` e aplicado depois com re-check; skip de recálculo de volume em som pausado). Runtime (video.js) APROVOU c/ ajuste (`_ytQueue.length=0` no destroy). Designer REPROVOU→corrigido (viz-pause + token de cor). Contrato REPROVOU→corrigido (R9). Este gate (revisão): sweep de regressão limpo (sem `set*Data`/`getVHub()`/spawn-native no wow; sem export órfão; sem módulo-fantasma; `video.js` em `files{}`).
    - **RESIDUAIS ACEITOS (não bloqueiam merge):** (i) **iframe_api CDN = exceção A-10 consciente** — `vhub_vehcontrol/html/index.html` carrega `https://www.youtube.com/iframe_api` (endpoint só servido por www.youtube.com; nocookie NÃO serve o iframe_api; o player em si aponta p/ nocookie no construtor). Tecnicamente NÃO há alternativa (Google não distribui a lib offline). **Formalmente requer ADR própria antes de ser considerada aprovada como padrão** (por ora = dívida registrada, mesma classe da exceção A-10 do wow na #52). (ii) `resetVideoToggle()` chamado por `video.js` via função GLOBAL (não pelo eventbus) — acoplamento A-03 menor, funcional no CEF flat, refactor futuro desejável. (iii) Fonte "Barlow Condensed" cai em equivalente de sistema; embarcar `.woff2` local é desejável (não bloqueia). (iv) camada espacial do áudio segue ANDAIME (herdado da #34/#52).
    - **TESTES RUNTIME PENDENTES (bloqueiam produção, não o merge):** dirigir → buscar YouTube → tocar → ligar "Vídeo no carro" (telinha monta em mute, áudio pelo wow) → sair do menu (telinha persiste) → trocar faixa (`refreshVideoIfOn`) → parar (telinha some); 2º player FORA do carro NÃO vê o DVD; soft-dep wow-off (rádio só falha, resto intacto); resmon idle 0 (engine morre no último som, viz pausa com NUI fechada); `PlayAtCoords` sem consumidor (superfície inerte). Rollback: `git checkout HEAD -- <13 arquivos modificados>` + `rm vhub_vehcontrol/html/video.js` (aditivo; reversão limpa, sem schema).
    - **CORE FROZEN:** não tocado (só `[SCRIPTS]`). **Próximo livre = #54.**

54. **`vhub_custom` — sprint de correção (bennys + mec): bug fixes + UX (2026-07-04 — GATE FINAL APROVADO; autonomia concedida, dono ausente). `[SCRIPTS]`, ZERO toque no CORE → sem ADR de CORE.** Endurece a oficina da #26 (bennys/mec/oficina construída). Escritor de verdade veicular segue `exports.vhub_conce:saveVehicleState` (Doutrina da Placa intacta) — esta sprint NÃO cria dado novo de veículo, só corrige fluxo/cobrança/UX.
    - **Arquivos (7, todos `[SCRIPTS]/vhub_custom`, ZERO toque no CORE, ZERO schema SQL novo):** `client/mec.lua`, `server/mec.lua`, `server/bennys.lua`, `web/mec.js`, `web/mec.css`, `web/bennys.js`, `client/zones.lua`.
    - **(1) Reparo com network-owner via `awaitControl(ent,timeout)` (`client/mec.lua`):** reparo de motor/lataria exige ser network owner do veículo p/ `CVehicleDamageSync` PERSISTIR — novo helper faz `NetworkRequestControlOfEntity` + loop com timeout (3000ms no reparo). Pneu NÃO precisa (`CVehicleTyreStateSync` propaga sem owner). Mesmo helper reusado no reboque `MEC_TOW_DO` (timeout 5000ms → reposiciona e persiste posição via `conce:updatePosition`). Correções de fluxo no mesmo arquivo: `activeVeh` capturado ANTES do `Wait(3000)` (fecha race), `closeMec`+`SendNUIMessage` disparados ANTES da thread de animação, replay-guard `_repairActive` liberado em TODOS os caminhos de saída (sem travar o player em falha).
    - **(2) `calcCost` derivado do patch validado (`server/bennys.lua`):** `calcCost(custPatch)` calcula preço sobre o `custPatch` JÁ passado por `buildCosmeticPatch` (whitelist) — NÃO cobra mods rejeitados pela whitelist. Toggles falsy (`smoke`/`xenon = false`) contam via `~= nil` (não somem da cobrança por serem `false`).
    - **(3) Health `nil` fail-explicit (`server/mec.lua`, L-03):** health lido do state `nil` agora vira ERRO EXPLÍCITO em vez de mascarar como "sem dano" (não presume 1000.0). Guarda DEFENSIVA — hoje é dead-code prático (o contrato `conce:getVehicleState` sempre normaliza p/ 1000.0), mantida deliberadamente como blindagem contra contrato futuro, alinhada L-03 (rollback p/ último estado válido, nunca inventar verdade).
    - **(4) `backdrop-filter` removido do `.mec-card` (`web/mec.css`, A-09):** CEF do FiveM renderiza `backdrop-filter` como bloco preto sólido sobre o mundo GTA — trocado por liquid glass em camadas (opacidade do piso ~0.88). +classes de barra de saúde.
    - **(5) Health bars na NUI (`web/mec.js`):** preço REAL exibido + barras de saúde de motor/lataria (feedback visual do dano antes do reparo).
    - **(6) `pushPreview` RAF-throttle + cleanup de listeners do palco (`web/bennys.js`, A-08/A-07):** preview do picker coalescido via `_previewRAF` (não dispara `SendNUIMessage` a 60fps no drag) — A-08. `_stageMoveRef`/`_stageUpRef` (listeners do palco de preview) removidos no `closeNUI` — A-07. `bindStage()` chamado 1x no boot do IIFE (sem listener duplicado).
    - **(7) Budget da thread quente renegociado (`client/zones.lua`, L-18):** thread com `Wait(16)` teve o orçamento (Hz/ms) declarado/renegociado no header — contrato de budget explícito (L-18).
    - **RISCOS RESIDUAIS / DÍVIDAS (não bloqueiam merge):** (i) a guarda de health-`nil` no `server/mec.lua` é DEAD-CODE prático hoje (o contrato `conce:getVehicleState` sempre normaliza p/ 1000.0) — mantida de propósito como defesa L-03 contra contrato futuro, não é regressão. (ii) `bennys.js`/`mec.js` são IIFE clássico, NÃO `vhub.createModule` → A-02 (lifecycle padronizado) e A-06 (native bridge centralizado) não plenamente atendidos: **dívida arquitetural PRÉ-EXISTENTE do resource** (herdada da #26, não introduzida por esta sprint). (iii) o hook L-15 (código morto) dá FALSO-POSITIVO em Windows por causa do path com colchetes `[SCRIPTS]` — verificação de manifest feita manualmente, A-10 OK.
    - **TESTES RUNTIME PENDENTES (bloqueiam produção, NÃO o merge):** (a) reparar motor/lataria quando o player NÃO é network owner do veículo — confirmar que `awaitControl` obtém controle DENTRO do timeout de 3000ms e que `SetVehicleEngineHealth`/`SetVehicleBodyHealth` PERSISTE; (b) reboque com `awaitControl` (timeout 5000ms) reposiciona e persiste posição via `conce:updatePosition`; (c) barras de saúde e A-09 (sem bloco preto do CEF sobre o mundo) confirmados no jogo.
    - **GREP DE FECHAMENTO:** L-13 limpo (nenhum `set*Data` fora do CORE — escreve via `saveVehicleState`), L-16 limpo (sem `SetPlayerModel`/`SetEntityCoords` de spawn fora do owner; `awaitControl`/`SetEntityCoords` do reboque = reposicionamento de veículo já existente sob controle, não spawn de ped). Sem export órfão, sem módulo-fantasma.
    - **CORE FROZEN:** não tocado (só `[SCRIPTS]`; sem schema SQL novo). **Rollback:** `git checkout HEAD -- resources/[SCRIPTS]/vhub_custom/` (aditivo/de correção — reversão limpa). **Próximo livre = #55.**

55. **Sprint remake visual "prime" — SOMENTE CSS nas 2 NUIs `vhub_custom/web` (Oficina/bennys/mec) e `vhub_vehcontrol/html` (2026-07-04 — GATE FINAL APROVADO; `vhub_designer` 9.2/10 + `vhub_guardiao_designer` 9/10 c/ ressalva de resmon + gate final `vhub_guardiao_revisao`; autonomia concedida, dono ausente). `[SCRIPTS]`, ZERO toque no CORE → sem ADR de CORE.** Refino cosmético, NÃO recriação (pedido do dono: "remake visual mais prime, não recriar"). Escopo ESTRITO: nenhum JS/Lua/HTML/evento/callback/export/schema/CORE tocado por esta decisão — só `.css`.
    - **Arquivos (7, todos `.css`):** `vhub_custom/web/style.css` (Oficina), `vhub_custom/web/bennys.css`, `vhub_custom/web/mec.css`; `vhub_vehcontrol/html/style-core.css`, `style-controls.css`, `style-ficha.css`, `style-sound.css`.
    - **(1) A-09 — últimas violações `backdrop-filter` sobre o jogo REMOVIDAS:** `vhub_custom/web/style.css` (era live nas ex-linhas 90 e 149) e `mec.css` (era live na ex-linha 33) tinham `backdrop-filter`/`-webkit-backdrop-filter` LIVE no HEAD anterior → no CEF do FiveM isso renderiza bloco preto sólido sobre o mundo GTA. Trocado por vidro SIMULADO em camadas (piso translúcido ≈0.86–0.90, dentro/na borda da faixa canônica A-09). Grep de fechamento: zero `backdrop-filter` LIVE nos 7 CSS (só comentários referenciando a ausência). `mec.css` piso ajustado 0.90→0.88.
    - **(2) PADRÃO REUTILIZÁVEL — guard de idle de animação (candidato a skill):** toda NUI que ESCONDE por `opacity:0` (NÃO `display:none`) mantém o elemento no render tree, então animações `infinite` continuam consumindo resmon com a UI fechada (idle > 0, viola espírito de A-07/L-18). Cravado o guard no wrapper de cada root oculto-por-opacity: `.<root>.hidden * { animation-play-state: paused !important; }`. Aplicado em `.mod-oficina.overlay.hidden` (style.css:119), `.vc-panel.hidden` (style-core.css:134) **e `.mod-mec.mec-overlay.hidden` (mec.css:25 — este ADICIONADO no gate final; era o gap do achado #1 do revisor, pois `vhub_custom/web` é documento ÚNICO `ui_page 'web/index.html'` e o guard do `.mod-oficina` não alcançava o root irmão `.mod-mec`)**. Cobre todas as animações `infinite`: tier-pulse, health-pulse, blink, sound-pulse, dvd-dot, viz-pulse. `style-sound.css` já pausa o visualizador via `soundViz.is-paused` (herdado da #53).
    - **(3) Fonte de display de sistema (A-10):** display migrado de Google Fonts CDN → Bahnschrift/DIN Condensed (fontes de sistema, ZERO CDN) — **DECISÃO DO DONO = manter**. Paga/reforça a dívida A-10 do vehcontrol (parcialmente quitada na #53). Grep A-10: zero `@import`/`@font-face`/`url()` externo nos 7 CSS; manifests INTACTOS (nenhum asset novo). "Barlow Condensed" cai em equivalente de sistema.
    - **(4) Keyframes escopados:** 20 keyframes prefixados por `mo-`/`bn-`/`mec-`/`vc-` — zero colisão de nome global. Estados de classe controlados por JS PRESERVADOS (custom: `active/hidden/mc-opt-disabled/on`; vehcontrol: `hidden/is-active/is-dragging/is-editing/is-on/is-paused/is-playing`) — nenhum seletor de estado JS removido.
    - **⚠️ NOTA DE MEMÓRIA — WORKING TREE COMINGLED (não é regressão desta sprint):** no momento deste registro a árvore de trabalho também carregava o sprint #53/#54 (vídeo "DVD do carro", `video.js` untracked, `<script src="https://www.youtube.com/iframe_api">`, `shared/events.lua`, `fxmanifest` v1.2.0, `vhub_wow`) — já GATED e aprovado, mas AINDA NÃO COMMITADO. Logo, um `git commit` neste ponto NÃO é puramente cosmético. **Commitar CSS SELETIVAMENTE (por arquivo `.css`), nunca `git add -A`.** A afirmação "só CSS" vale para ESTA decisão, não para o estado bruto da árvore.
    - **TESTES RUNTIME PENDENTES (não bloqueiam merge; bloqueiam produção):** (a) **idle = 0 em resmon com NUI FECHADA nas 2 UIs** — especialmente reabrir/fechar o mec com uma barra `mc-health-low` visível (o caso exato que vazava antes do guard `mec.css:25`); (b) legibilidade do vidro simulado (piso 0.86–0.90) sobre cena GTA clara E escura in-game (contraste do texto dourado/branco).
    - **CORE FROZEN:** não tocado (só `[SCRIPTS]`; sem schema SQL novo). **Rollback (por-arquivo, NÃO por-pasta):** os CSS/HTML do #53/#54 vivem nas MESMAS pastas — `git checkout HEAD -- resources/[SCRIPTS]/vhub_custom/web/ "resources/[SCRIPTS]/vhub_vehcontrol/html/"` reverteria também o feature sprint. Para reverter só o remake visual sem perder #53/#54, usar `git checkout HEAD -- <arquivo.css>` por arquivo. **Próximo livre = #56.**
