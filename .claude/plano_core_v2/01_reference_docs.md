...
O `sss.txt` é a **filosofia matemática original** (categorias com `fMass`/`fDriveBiasFront`/`fTractionMax`). O `PLANO_IMPLEMENTACAO_VEICULOS.md` **(SUPERSEDED 2026-07-03 — ver o próprio arquivo)** propunha um **Self-Healing em runtime** (`boot_service.lua` no `vhub_conce`) que clamparia veículos violadores no boot do servidor.
...
> **⚠️ SUPERSEDED 2026-07-03.** O documento original foi arquivado por desalinhamento com a
> arquitetura real (`MySQL.Async` inexistente, tabela `player_vehicles` inexistente, ownership
> de handling no conce viola L-04/R4). A seção abaixo permanece como corpus de auditoria
> histórico; a intenção legítima (clamp físico por arquétipo) vive em `frozen_core_2.md`
> §F-053/§F-068 e ADR #51 proposta. Ownership do clamp = `vhub_vehcontrol`, não conce.
