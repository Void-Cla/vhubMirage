...
>
> **⚠️ Numeração aplicada vs. proposta (2026-07-03):** as referências `F-XXX` e as ADRs do
> Apêndice B são a numeração **proposta** por este plano. A execução real está registrada em
> `WORKLOG_CORE_V2.md` (ADRs #37..#50 aplicadas, próximo livre #51) e no `contexto.md`. Onde
> houver divergência de numeração, **o WORKLOG prevalece** — ver nota no topo do Apêndice B.
...
> **⚠️ Numeração drift (reconciliação 2026-07-03).** As entradas #36..#55 abaixo são a
> numeração **proposta** por este apêndice, escrita antes da execução. A numeração
> **efetivamente aplicada** vive em `WORKLOG_CORE_V2.md` e no `contexto.md` — e diverge:
> o WORKLOG aplicou ADRs **#37..#50** (com semântica parcialmente diferente; ex.: aqui
> #37 = `commitVehicleState`, mas no WORKLOG #37 = reanimação gated de handlers). O
> **próximo ADR livre é #51** — sempre confirmar no WORKLOG antes de numerar. Em caso de
> divergência, **o WORKLOG prevalece**; estas linhas permanecem só como proposta histórica.
