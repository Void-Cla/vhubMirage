...
□ L-01..L-19 e A-01..A-08 respeitadas? □ Linha do Registro de Ownership presente/atualizada se toca dado? □ Grep de fechamento limpo (`set*Data` externo | `getVHub` p/ escrita | `SetEntityCoords/SetPlayerModel` de spawn fora do owner)? □ Nenhum arquivo órfão do manifest; nenhum módulo-fantasma; deleções acompanham criações (L-15)? □ Replay-guard em handlers de `playerSpawn/characterLoad` (L-17)? □ `assertThread` com `Await`; exports sensíveis com `_invoker_allowed()`; sem `print` fora de logger/bootstrap? □ Orçamentos: resmon antes/depois quando toca hot path (L-18)? □ Smoke test executável + rollback de 1 linha descritos? □ DESCONGELAMENTO: toque em `[CORE]/vhub/**` tem **ADR numerada + bump de versão + co-gate do arquiteto**? Migração para o CORE **não deixa 2ª fonte de verdade viva** em `[SCRIPTS]` (deprecation path + migration, R15)? ADR nova → registrar no `contexto.md`.
